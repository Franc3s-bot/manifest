...
describe('RequestVolumeService (#2511 request-level volume)', () => {
  ...
  it('defaults the upper bound and splits at $4 when no agent is scoped', async () => {
    ...
    expect(windows).toEqual({
      current: { total: 5, success: 3, healed: 0, keyRotation: 0, fallback: 0, error: 2 },
      previous: { total: 5, success: 4, healed: 0, keyRotation: 0, fallback: 0, error: 1 },
    });
    const sql = lastSql();
  ...
  it('splits current and previous disposition totals from ONE scan', async () => {
    ...
    expect(windows).toEqual({
      current: { total: 100, success: 70, healed: 4, keyRotation: 0, fallback: 6, error: 20 },
      previous: { total: 98, success: 60, healed: 3, keyRotation: 0, fallback: 5, error: 30 },
    });
  ...
  it('returns empty current and previous windows without a tenant, never querying', async () => {
    await expect(
      ...
      current: { total: 0, success: 0, healed: 0, keyRotation: 0, fallback: 0, error: 0 },
