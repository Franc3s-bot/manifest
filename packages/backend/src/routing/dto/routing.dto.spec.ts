...
describe('SetFallbacksDto', () => {
  ...
  it('should pass with up to 15 model strings', async () => {
    const dto = toFallbacksDto({
      models: Array.from({ length: 15 }, (_, i) => `model-${i + 1}`),
    });
    const errors = await validate(dto);
    expect(errors).toHaveLength(0);
  });
  ...
  it('should reject more than 15 items', async () => {
    const dto = toFallbacksDto({
      models: Array.from({ length: 16 }, (_, i) => `model-${i + 1}`),
    });
    const errors = await validate(dto);
    expect(errors.length).toBeGreaterThan(0);
  });
